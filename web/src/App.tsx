...
function FloorMap({ floor, spots, visibleIds, selectedId, onSelect }: { floor: FloorPlanFloor; spots: ParkingSpot[]; visibleIds: Set<string>; selectedId?: string; onSelect: (spot: ParkingSpot) => void }) {
  ...
  return <div className="floor-map-wrap"><svg className="floor-map" viewBox={`0 0 ${floor.canvas.width} ${floor.canvas.height}`} aria-label={`${floor.name} interactive floor plan`}><rect className="map-shell" x="2" y="2" width={floor.canvas.width - 4} height={floor.canvas.height - 4} rx="2" />{floor.features.map((feature) => <g key={feature.id}><polyline className={`map-feature ${feature.type}`} points={points(feature.points)} />{feature.type === 'gate' && <text className="map-gate" x={(feature.points[0].x + feature.points[1].x) / 2} y={(feature.points[0].y + feature.points[1].y) / 2 - 2}>{feature.direction === 'in' ? 'IN' : 'OUT'}</text>}</g>)}{floor.bays.map((bay) => { const values = bay.spots.map((spot) => spot.geometry); const minX = Math.min(...values.map((item) => item.x)); const minY = Math.min(...values.map((item) => item.y)); const maxX = Math.max(...values.map((item) => item.x + item.width)); const maxY = Math.max(...values.map((item) => item.y + item.height)); return <g key={bay.id}><rect className="map-bay" x={minX - 3} y={minY - 4} width={maxX - minX + 6} height={maxY - minY + 8} /><text className="map-bay-label" x={minX - 2} y={minY - 1}>{bay.name}</text></g>; })}{floor.bays.flatMap((bay) => bay.spots).map((planSpot) => { const spot = byId.get(planSpot.id); if (!spot) return null; const geometry = planSpot.geometry; const cx = geometry.x + geometry.width / 2; const cy = geometry.y + geometry.height / 2; const hidden = !visibleIds.has(spot.id); const status = spot.status === 'available' ? 'available' : spot.occupancySource === 'manual' ? 'manual' : 'occupied'; return <g key={spot.id} className={`map-space ${status} ${hidden ? 'muted' : ''} ${selectedId === spot.id ? 'selected' : ''}`} transform={`rotate(${geometry.orientation} ${cx} ${cy})`} role="button" tabIndex={0} aria-label={`${spot.number}, ${status}`} onClick={() => onSelect(spot)} onKeyDown={(event) => activate(event, spot)}><rect x={geometry.x} y={geometry.y} width={geometry.width} height={geometry.height} rx="0.8" /><text x={cx} y={cy + 0.8}>{spot.number}</text></g>; })}</svg></div>;
}
...
function LayoutPage({ client }: { client: ApiClient }) {
  ...
  const previewSpots = previewFloor?.bays.flatMap((bay) => bay.spots.map((spot) => ({ id: spot.id, floorId: previewFloor.id, bayId: bay.id, number: spot.number, status: 'available' as const }))) ?? [];
